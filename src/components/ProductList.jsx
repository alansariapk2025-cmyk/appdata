...
export default function ProductList(){
  ...
  const fetchProducts=useCallback(async(loadMore=false)=>{
    ...
    try{
      const q=loadMore&&lastDoc
        ?query(collection(db,"products"),orderBy("createdAt","desc"),startAfter(lastDoc),limit(FETCH_LIMIT))
        :query(collection(db,"products"),orderBy("createdAt","desc"),limit(FETCH_LIMIT));
      const snap=await getDocs(q);
      const docs=snap.docs.map(d=>({id:d.id,...d.data()}));
      setAllProducts(prev=>loadMore?[...prev,...docs]:docs);
      setLastDoc(snap.docs[snap.docs.length-1]||null);
      setHasMoreServerData(snap.docs.length===FETCH_LIMIT);
      setIsConnected(true);
    }catch(e){
  ...
  const handleRefresh=async()=>{
    setLastDoc(null); setHasMoreServerData(true); setSelectedProducts([]); await fetchProducts(false);
  };
  ...
  const toggleStatus=async(product)=>{
    ...
    try{
      ...
      setAllProducts(prev=>prev.map(p=>p.id===product.id?{...p,status:newStatus}:p));
  ...
  const handleSave=async()=>{
    ...
    try{
      ...
      setAllProducts(prev=>prev.map(p=>p.id===editProduct.id?{...p,...payload}:p));
  ...
  const handleBulk=async(action)=>{
    ...
    try{
      ...
      await batch.commit();
      setAllProducts(prev=>{
        if(action==="delete")return prev.filter(p=>!selectedProducts.includes(p.id));
        return prev.map(p=>selectedProducts.includes(p.id)?{...p,status:action==="activate"?"active":"inactive"}:p);
      });
      setSelectedProducts([]);
  ...
  const exportData=(type,format)=>{
    const data=(type==="all"?allProducts:filteredProducts).map(p=>({
      ...
      "Discount %":num(p.discount),Stock:num(p.stock),Unit:p.unit||"",Status:p.status||"inactive",Popular:p.mostPopular?"Yes":"No",
        ...
        data.forEach(row=>{
          ...
          if(name){
            batch.set(doc(collection(db,"products")),{
              ...
              price:num(row.price||row["Price (PKR)"]),mrpPrice:num(row.mrpPrice||row.MRP),discount:num(row.discount||row["Discount %"]),
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-4 sm:p-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={handleRefresh} className="flex items-center gap-1.5 px-3 py-2 bg-white rounded-lg font-semibold text-sm shadow"><FaSync className={`w-4 h-4 ${loadingMore?"animate-spin":""}`}/> Refresh</button>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        ...
        {hasMoreServerData&&<div className="px-4 py-4 border-t bg-white">
          <button onClick={()=>fetchProducts(true)} disabled={loadingMore} className="w-full py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl font-semibold shadow disabled:opacity-50 flex items-center justify-center gap-2">
            {loadingMore?<><FaSync className="animate-spin w-4 h-4"/> Loading...</>:<><FaChevronRight className="w-4 h-4"/> Load More Products</>}
          </button>
        </div>}
      </div>
      ...
      {editProduct&&<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
          ...
          <div className="p-4 overflow-y-auto max-h-[calc(90vh-120px)] grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs font-semibold text-gray-600">Discount %</label><input type="number" value={editProduct.discount||""} onChange={e=>setEditProduct({...editProduct,discount:e.target.value})} className="w-full p-2 border rounded-lg mt-1"/></div>
